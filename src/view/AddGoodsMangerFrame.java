...
public class AddGoodsMangerFrame extends JInternalFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AddGoodsMangerFrame frame = new AddGoodsMangerFrame();
    ...
    public AddGoodsMangerFrame(){
		...
		JLabel label_date = new JLabel("入库时间：");
		...
		modifyBtn.setIcon(new ImageIcon(AddGoodsMangerFrame.class.getResource("/images/modify.png")));
		//删除按钮
		...
		deleteBtn.setIcon(new ImageIcon(AddGoodsMangerFrame.class.getResource("/images/delete.png")));
		...
		JLabel labelDate = new JLabel("入库时间：");
		first_goodsDateText = new JTextField();
		first_goodsDateText.addFocusListener(new JTextFieldHintListener(first_goodsDateText, "格式：yyyy-mm-dd"));
		...
		s_searchBtn.setIcon(new ImageIcon(AddGoodsMangerFrame.class.getResource("/images/search.png")));
		...
		//初始化表格显示，显示所有的书籍
		fillGoodsTable(new Warehousing());
    }
	...
	private void modifyGoodsActionPerformed(ActionEvent evt) {
		...
		if(goodsDate==null || "".equals(goodsDate)){ //为空
			JOptionPane.showMessageDialog(null, "入库时间不能为空！");  //给用户提示
			return;
		}
		//从获取的货物信息创建货物对象
		Warehousing warehousing=new Warehousing(Integer.parseInt(id),goodsDate,goodsName, Integer.parseInt(goodsCategory), Integer.parseInt(goodsNumber));
		//定义数据库连接
		Connection con=null;
		try {
			//获取数据库连接
			...
			int res=warehousingDao.update(con, warehousing);
			if(res==1){ //为1
	...
	private void deleteGoodsActionPerformed(ActionEvent evt) {
		...
		try {
			...
			int res=warehousingDao.delete(con, Integer.parseInt(id));
			if(res==1){ //为1
	...
	private void searchActionPerformed(ActionEvent evt) {
		...
		warehousing.setGoodsDate(goodsDate);
		//调用table填充函数，根据查询结果重新填充表格
		fillGoodsTable(warehousing);
	}
	/**
	 * 初始化表格，列出所有的信息
	 * @param warehousing
	 */
	private void fillGoodsTable(Warehousing warehousing){
		//获取表格的模型
		...
		try {
			...
			warehousingDao=new WarehousingDao();
			//按条件查询（这里没有条件，查出所有信息）
			ResultSet rs=warehousingDao.search(con, warehousing);
			//遍历查询结果
			while(rs.next()){
				//定义一个集合，由于存储图书信息
				Vector v=new Vector();
				v.add(rs.getInt("warehousing_record_id"));
				v.add(rs.getString("goods_name"));
				v.add(rs.getInt("goods_category"));
				v.add(rs.getInt("receipt_quantity"));
